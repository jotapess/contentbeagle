...
import { Plus, Search, Shield, User, Filter, Loader2 } from "lucide-react";

import type { CombinedRule } from "@/lib/actions/ai-rules";
import { toggleRuleActive } from "@/lib/actions/ai-rules";
...
function RuleRow({
  ...
  rule: CombinedRule;
  teamId: string;
  onToggle: (ruleId: string, isGlobal: boolean, newState: boolean) => void;
    ...
    <TableRow>
      ...
      <TableCell>
        <span className="text-sm text-muted-foreground">
          {patternTypeLabels[rule.pattern_type] || rule.pattern_type}
        </span>
      </TableCell>
      <TableCell>
      ...
      <TableCell>
        {isToggling === rule.id ? (
          ...
          <Switch
            ...
            onCheckedChange={(checked) => onToggle(rule.id, rule.is_global, checked)}
...
export function AIRulesClient({ rules: initialRules, teamId }: AIRulesClientProps) {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = React.useState("");
  const [categoryFilter, setCategoryFilter] = React.useState<string>("all");
  const [rules, setRules] = React.useState<CombinedRule[]>(initialRules);
  const [togglingRule, setTogglingRule] = React.useState<string | null>(null);

  const globalRules = React.useMemo(() => {
    return rules.filter((r) => r.is_global);
  }, [rules]);
  ...
  async function handleToggle(ruleId: string, isGlobal: boolean, newState: boolean) {
    setTogglingRule(ruleId);

    // Optimistic update
    setRules((prev) =>
      prev.map((r) => (r.id === ruleId ? { ...r, is_active: newState } : r))
    );

    const { error } = await toggleRuleActive(teamId, ruleId, isGlobal, newState);
  ...
  const globalActiveCount = globalRules.filter((r) => r.is_active).length;
  const customActiveCount = customRules.filter((r) => r.is_active).length;
