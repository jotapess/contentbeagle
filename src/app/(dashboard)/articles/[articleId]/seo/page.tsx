...
import { getArticleById, mockKeywordData } from "@/lib/mock-data";
...
export default function SEOPage() {
  ...
  const [metaTitle, setMetaTitle] = React.useState(article?.seoTitle ?? "");
  const [metaDescription, setMetaDescription] = React.useState(
    article?.seoDescription ?? ""
  );

  const seoScore = article?.seoScore ?? 72;

  const focusKeywordVolume = React.useMemo(() => {
    const data = mockKeywordData.find(
  ...
  const seoChecks: SEOCheckItem[] = React.useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        passed: (article?.appliedLinks?.length ?? 0) >= 2,
